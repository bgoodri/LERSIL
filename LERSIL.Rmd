...
## Directed Acyclic Graph from 10-25-2014
...
writeLines(readLines(file.path("src", "stan_files", "LERSIL.stan"))[35:52])
```

## `parameters` Block of a Stan Program

```{r, comment = NA, echo = FALSE}
writeLines(readLines(file.path("src", "stan_files", "LERSIL.stan"))[157:173])
```

## `transformed parameters` Block of a Stan Program

```{r, comment = NA, echo = FALSE}
writeLines(readLines(file.path("src", "stan_files", "LERSIL.stan"))[174:183])
```
...
```{r, comment = NA, echo = FALSE}
writeLines(readLines(file.path("src", "stan_files", "LERSIL.stan"))[184:206])
```

## `generated quantities` Block of a Stan Program

```{r, comment = NA, echo = FALSE}
writeLines(readLines(file.path("src", "stan_files", "LERSIL.stan"))[218:232])
```
where it draws from the posterior distribution of $\boldsymbol{\eta}$ and $\boldsymbol{\xi}$ if
